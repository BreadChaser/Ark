...
export function agentStateFromScreen(session, text, controls = parseAgentControls(session?.tool, text)) {
  ...
  const lines = parseTerminalLines(text).slice(-16).map((line) => line.text);
  const prompt = lines.findLastIndex((line) => /^›\s+\S/.test(line));
  if (prompt !== -1 && !/(?:Working|Thinking|Reasoning)(?:\.{3}|…|\s*\()|esc to interrupt|Running\s+\w+.*hook|waiting for (?:a tool|tool output|agents?)/i.test(lines.slice(prompt + 1).join("\n"))) return "ready";
  const screen = lines.join("\n");
  return /(?:Working|Thinking|Reasoning)(?:\.{3}|…|\s*\()|esc to interrupt|Running\s+\w+.*hook|waiting for (?:a tool|tool output|agents?)/i.test(screen)
    ? "working"
    : "ready";
}
...
function activePromptLines(lines) {
  ...
  const start = lines.findLastIndex((line, index) => index <= end && (/^(?:Would you|Do you|Allow\b.*\?|Update Model Permissions$|Update\b.*\?|Select Model and Effort$|Select Reasoning Level\b|Choose\b)/i.test(line)
    || /\b(?:requires?|requested)\b.*\b(?:approval|confirmation|permission)\b/i.test(line)));
