...
export function parseCodexControls(text, lines = parseTerminalLines(text)) {
  ...
  if (/Would you like to run the following command\?/i.test(compact)) return [commandApprovalControl(cleanLines)];
  if (/Update Model Permissions|\/permissions\s+choose|Ask for approval \(current\)|Approve for me/i.test(compact)) return [permissionsControl()];
