...
try {
  ...
  await js(`
    renderNetworkSites({ scanned_at: "2026-07-18T12:00:00.000Z", hosts: 254, pinned: [{ url: "http://192.168.1.50:8787/reports/cleanup.html", name: "Cleanup report", kind: "Pinned page", pinned: true }], sites: [{ url: "http://192.168.1.50:8096/", name: "Jellyfin", kind: "Media server", device: "Jelly machine", status: 200 }], hidden: [{ url: "http://192.168.1.1:8080/", name: "Router", kind: "Web dashboard", status: 200 }] });
    document.querySelector("#network-sites-dialog").showModal();
    return true;
  `);
  await wait('document.querySelector("#network-sites-dialog").open');
  assert(await js('const links = [...document.querySelectorAll("#network-sites-list a")]; const text = document.querySelector("#network-sites-list").textContent; return Boolean(document.querySelector("#network-sites-toggle")) && Boolean(document.querySelector("#network-sites-pin-form")) && Boolean(document.querySelector("[data-network-site-rename]")) && Boolean(document.querySelector("[data-network-site-hide]")) && Boolean(document.querySelector("[data-network-site-show]")) && text.includes("Pinned pages") && text.includes("Hidden sites (1)") && text.includes("Cleanup report") && text.includes("Media server") && text.includes("Jelly machine") && text.includes("192.168.1.50:8096") && links.some((link) => link.href.includes("8787/reports/cleanup.html")) && links.every((link) => link.target === "_blank" && link.rel.includes("noopener"));'), "network sites do not preserve or safely open pinned dashboards");
...
async function testCodexControlPrompts() {
  ...
  await runLocal("tmux", ["respawn-pane", "-k", "-t", tmuxName, "bash"]);
  await tmuxSendLine(tmuxName, `bash ${JSON.stringify(path.resolve("test/fixtures/controls/menu-harness.sh"))} safety`);
  ...
  assert(await js('return document.querySelector("#control-prompt").textContent.includes("Additional safety checks") && document.querySelector("#control-body").textContent.includes("Keep waiting") && document.querySelector("#control-body").textContent.includes("Learn more");'), "Codex safety-check prompt was not shown in the GUI");
  await js('document.querySelector("#control-body [data-command=\\"1\\"]").click(); return true;');
