...
try {
  ...
  const doneSound = await fetch(`${BASE_URL}/static/done.wav?v=3`);
  ...
  assert(await js('return Boolean(document.querySelector("#test-done-sound") && document.querySelector("#test-input-sound") && document.querySelector("#sound-volume"));'), "sound controls are missing from settings");
...
async function testCodexControlPrompts() {
  ...
  await js('activeSession().codex_usage = { plan_type: "pro", primary: { used_percent: 91, resets_at: 1783760535 }, secondary: { used_percent: 36, resets_at: 1784310183 } }; renderCodexFooter(); return true;');
  assert(await js('return document.querySelector("#codex-footer").textContent.includes("91% used") && document.querySelector("#codex-footer").textContent.includes("36% used") && document.querySelectorAll("#codex-footer progress").length === 2 && document.querySelector(".codex-usage-warning")?.textContent.includes("nearly exhausted");'), "Codex account footer lost usage limits or warning");
  assert(await js('return document.querySelector("[data-auto-resume]")?.getAttribute("aria-pressed") === "false";'), "Codex auto-resume toggle is unavailable");
  const expiredReset = new Date().toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true });
  await tmuxSendLine(tmuxName, `printf "You've hit your usage limit. Try again at ${expiredReset}.\\nGoal hit usage limits (/goal resume)\\n"; read -r line; printf 'auto-resume:%s\\n' "$line"`);
  await waitForTerminalLogText(disposableSession.id, "Goal hit usage limits (/goal resume)");
  await js('document.querySelector("[data-auto-resume]").click(); return true;');
  await wait('document.querySelector("[data-auto-resume]")?.getAttribute("aria-pressed") === "true"');
  await shot("codex-usage-warning");
  assert((await api("/api/sessions")).sessions.find((session) => session.id === disposableSession.id)?.auto_resume_goal === true, "Codex auto-resume setting was not persisted");
  await waitForTerminalLogText(disposableSession.id, "auto-resume:/goal resume");
  assert((await api("/api/sessions")).sessions.find((session) => session.id === disposableSession.id)?.auto_resume_last_reset > 0, "Codex auto-resume did not record the reset");
  await js('document.querySelector("[data-auto-resume]").click(); return true;');
  await wait('document.querySelector("[data-auto-resume]")?.getAttribute("aria-pressed") === "false"');
